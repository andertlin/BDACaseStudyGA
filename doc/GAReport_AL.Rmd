...
```{r, echo=FALSE, message=FALSE, prompt=FALSE, results='asis'}
str(gadata)
gadata.m <- melt(gadata, id=c(1:8), measure= c(9:14))
total_data <- cast(gadata.m, pagePath ~ variable, sum)
str(total_data)
names(total_data) <- c("pagePath", "total_entrance", "total_pageviews", "total_exits", "total_timeOnPage", "total_uniquePageviews", "total_users")
subset_datanocpc <- cast(gadata.m, pagePath ~ variable, sum, subset=((Medium!="cpc")))

total_pageview <- subset(total_data, select=c(1, 3))
subset_pageview <- subset(subset_datanocpc, select=c(1,3))
comparison_pageview <- merge(total_pageview, subset_pageview, by = "pagePath")
```
...
total_data[, "exit_rate"] <- total_data$total_exits / total_data$total_pageviews
exit_summary <- subset(total_data, select = c(1, 4, 8))
...
total_data[, "avg_timeOnPage"] <- total_data$total_timeOnPage / (total_data$total_pageviews - total_data$total_exits)
timeOnPage_summary <- subset(total_data, select = c(1, 9))
...
str(gadata)
gadata.m <- melt(gadata, id=c(1:8), measure= c(9:14))
gadata.m[, "combo"] <- paste(gadata.m$source, gadata.m$Medium, sep = "")
total_combonewv <- cast(gadata.m, combo ~ variable, sum, subset=((userType=="New Visitor")))
total_combonewv[, "exit_rate"] <- total_combonewv$exits / total_combonewv$pageviews
total_comboretv <- cast(gadata.m, combo ~ variable, sum, subset=((userType=="Returning Visitor")))
total_comboretv[, "exit_rate"] <- total_comboretv$exits / total_comboretv$pageviews
...
gadata.m <- melt(gadata, id=c(1:8), measure= c(9:14))
str(gadata.m)
total_spageviews <- cast(gadata.m, source ~ variable, sum, subset=((Medium=="referral")))
